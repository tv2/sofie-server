...
export class ActionController extends BaseController {
  ...
  public async getActions(request: Request, response: Response): Promise<void> {
    try {
      ...
      response.send(this.responseFormatter.buildSuccessResponse({ actions: actions.map(action => new ActionDto(action)) }))
    } catch (error) {
  ...
  public async executeAction(request: Request, response: Response): Promise<void> {
    try {
      ...
      response.send(this.responseFormatter.buildSuccessResponse({ message: `Successfully executed action: ${actionId} on Rundown: ${rundownId}` }))
    } catch (error) {
      this.httpErrorHandler.handleError(response, error as Exception)
    }
  }
